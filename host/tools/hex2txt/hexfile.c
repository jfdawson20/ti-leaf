 ...
 *
 * Modified 2005-01-18 by Tom Torfs: now type 3 records (start address record),
 * such as produced by msp430-objcopy -O ihex, are ignored.
 *
	...
	*filename)
{
	...
	if (hf->mem==NULL)	// error handling added by TT 2005-01-18
		return -1;
	hf->flag = malloc(memsize);
	if (hf->flag==NULL)	// error handling added by TT 2005-01-18
		return -1;
	hf->hl = malloc(sizeof(struct hexline));
	if (hf->hl==NULL)	// error handling added by TT 2005-01-18
		return -1;
 ...
 * Added by TT: now ignores valid start address records (type 03).
 *
 ...
 *           1 on EOF record
 *			 3 on start address record (to be ignored)
...
int parse_line(struct hexfile *hf)
{
	...
	else if (hl->record == 3 && hl->len==4 && hl->addr==0)		/* added by TT 2005-01-18 */
		return 3;			/* valid start address record, tell caller to ignore */ 
	else if (hl->record != 0)
               ...
               void(*cb)(const struct hexline *, void *), void *arg)
{
	...
	while (!feof(hf->ifile))
	{
		...
		switch(record)
		{
			...
			case 3:  /* start address record, do nothing - added by TT 2005-01-18 */
				break;
